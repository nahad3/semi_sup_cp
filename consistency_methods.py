...
if ladder_var == 1:
    ...
    optimizer_joint_train = optim.Adam(list(modelAE.parameters()) + list(model_label.parameters()), lr=0.001)
elif semi_sup == 1:
    temp = 10  # Mackay
    if case == 6 or case == 1:
        temp = 5  # Mean var
...
def train_feedforward_semi_vat(l_batch, u_batch, lam_l, lam_u):
    ...
    for i in list(range(0, max(no_unlabel, no_label))):
        ...
        loss_pi_unlb = 0
        loss_pi_lab = 0
        optimizer_joint_train.zero_grad()
        ...
        if i <= no_unlabel - 1:
            ...
            X2_recon, X2_enc = model(X_u)
            X2_enc = X2_enc.transpose(1, 2)  # .view(-1,output_size_tcn) #comment out for AE setting
            X2_enc = F.normalize(X2_enc, p=2, dim=2)
            if mean_emb == 1:
                X2_enc = torch.mean(X2_enc, dim=1)

            recon,enc_perturb = modelAE(X_u + rand_u)
            enc_perturb = enc_perturb.transpose(1, 2)  # .view(-1,output_size_tcn) #comment out for AE setting
            enc_perturb = F.normalize(enc_perturb, p=2, dim=2)
            if mean_emb == 1:
                enc_perturb= torch.mean(enc_perturb, dim=1)
            X_u_prob = model_label(X2_enc)
            X_u_prob_pert = model_label(enc_perturb)

            for param in model_label.parameters():
                param.requires_grad = False
            for param in modelAE.parameters():
                param.requires_grad = False
            loss_emb = criterion_MSE(torch.log(X_u_prob.detach()), X_u_prob_pert)
            loss_emb.backward()
...
def train_feedforward_semi_pi(l_batch, u_batch, lam_l, lam_u):
    mean_emb = 1
    ...
    for i in list(range(0, max(no_unlabel, no_label))):
        ...
        if i <= no_unlabel - 1:
            ...
            X1_recon, X2_enc_noisy = modelAE(X_u)
            X2_enc_noisy  = X2_enc_noisy .transpose(1, 2)  # .view(-1,output_size_tcn) #comment out for AE setting
            X2_enc_noisy  = F.normalize(X2_enc_noisy , p=2, dim=2)
            if mean_emb == 1:
                X2_enc_noisy  = torch.mean(X2_enc_noisy, dim=1)
            X_u_prob_pert = model_label(X2_enc_noisy)


            loss_pi_unlb = criterion_MSE(X_u_prob, X_u_prob_pert )
        ...
        if i <= no_label - 1:
            ...
            X1_recon, X2_enc = modelAE(X_l)

            X2_enc = X2_enc.transpose(1, 2)  # .view(-1,output_size_tcn) #comment out for AE setting
            X2_enc = F.normalize(X2_enc, p=2, dim=2)
            if mean_emb == 1:
                X2_enc = torch.mean(X2_enc, dim=1)
            X_l_prob = model_label(X2_enc)
        ...
        loss = (lam_u * loss_pi_unlb)  + loss_label
        # loss = loss_label
        if loss != 0:
            loss.backward()
            optimizer_joint_train.step()
...
if __name__ == "__main__":
    ...
    for ep in range(0,2000):
        lam_l = 0
        lam_u = 0.01# train_feedforward_semisup(balanced_train_btch, pair_train_btch,lam_u, lam_label)
        train_feedforward_semi_pi(balanced_train_btch,ul_data_batch,lam_l,lam_u)
