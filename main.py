...
from data_loader import  LabelledDataset, get_uniform_samples, CreateDataset, CreateDataset4mDict,ConstraintDataset4mCP, SegmentedLabelledDataset
...
train_set_pair, val_set_pair = random_split(pair_data_train,[ int(train_ratio_pair  * len(pair_data_train)), \
                                                              len(pair_data_train) - int( train_ratio_pair  * len(pair_data_train))])
...
unlabelled_data, val_set_lab   = random_split(val_set_lab,[ int(ul_split* len(val_set_lab)), len(val_set_lab) - int(ul_split* len(val_set_lab) )])
...
if ladder_var == 1:
    ...
    model1 = TCN(input_size, output_size_tcn, n_channels, kernel_size, temp, dropout, win_size, 1)
    ...
    optimizer_classifier = optim.Adam(model_label.parameters(), lr=0.001)
    ...
    for i in list(range(0,max(no_label,no_pair))):
        ...
        if i <= no_label -1:
            ...
            if plot_emb == 1:
                ...
                plt.scatter(X_0[:, 0], X_0[:, 1],  facecolors = 'none',edgecolors=[(0, 0.4470, 0.7410)], s= 100,label='Class 1')
                plt.scatter(X_1[:, 0], X_1[:, 1], facecolors = 'none',edgecolors=[(0.9290, 0.6940, 0.1250)],  s=100,label='Class 2')
                plt.scatter(X_2[:, 0], X_2[:, 1],  facecolors = 'none',edgecolors=[(0.8500, 0.3250, 0.0980)], s=100,label='Class 3')
                plt.scatter(X_3[:, 0], X_3[:, 1], facecolors = 'none',edgecolors=[(0.4940, 0.1840, 0.5560)], s=100,label='Class 4')
...
def semi_sup_train_autoenc(u_batch,l_batch, lam_u,lam_l, ep):
    ...
    for i in list(range(0, max(no_unlabel,no_label))):
        ...
        if i <= no_label - 1:
            ...
            #loss_label = criterion_CrsEnt(X_prob, Y)
            loss_label = criterion_CrsEnt(X_prob.transpose(1,2), Y.squeeze(2))
            if ep == 999:
                plot_emb = 1
            else:
                plot_emb = 0
            ...
            if plot_emb == 1:
                ...
                plt.scatter(X_0[:, 0], X_0[:, 1], facecolor = 'none',edgecolor=[(0, 0.4470, 0.7410)], label='Class 1',s=100)
                plt.scatter(X_1[:, 0], X_1[:, 1], facecolor = 'none',edgecolor= [(0.9290, 0.6940, 0.1250)], label='Class 2',s = 100)
                plt.scatter(X_2[:, 0], X_2[:, 1],  facecolor = 'none',edgecolor=[(0.8500, 0.3250, 0.0980)] \
                            , label='Class 3',s=100)
                plt.scatter(X_3[:, 0], X_3[:, 1], facecolor='none', edgecolor= [(0.4940, 0.1840, 0.5560)], label='Class 4',s=100)
                ...
                X_4_u = tsne[Y_l_l == 4, :]
                X_5_u = tsne[Y_l_l == 5, :]
                plt.scatter(X_0_u[:, 0], X_0_u[:, 1], facecolor = 'none', edgecolor=[(0, 0.4470, 0.7410)], label='Class 1')
                plt.scatter(X_1_u[:, 0], X_1_u[:, 1], facecolor = 'none',edgecolor=[(0.9290, 0.6940, 0.1250)], label='Class 2')
                plt.scatter(X_2_u[:, 0], X_2_u[:, 1], facecolor = 'none',edgecolor=[(0.8500, 0.3250, 0.0980)], label='Class 3')
                plt.scatter(X_3_u[:, 0], X_3_u[:, 1], facecolor = 'none',edgecolor=[(0.4940, 0.1840, 0.5560)], label='Class 4')
                ...
                plt.xlabel('t-SNE dim 1')
                plt.ylabel('t-SNE dim 2')
                plt.savefig('./data_files/gen_simulations/mat_files/mackay_switch/AE_MGlass_20_labels_true_lab.pdf', dpi=300 \
                            , format='pdf')
                plt.show()
...
if __name__ == "__main__":
    balanced_train = get_balanced_samples(train_set_lab,train_samples,output_size)
    val_balanced_test = get_balanced_samples(train_set_lab,1000,output_size)
    balanced_train_btch = DataLoader(balanced_train,batch_size= batch_size_label, shuffle=True)
    ...
    if ae ==1:
        ...
        torch.save(model_label.state_dict(),
                   '/home/nahad/pycharm_mac/semi_with_cp/semi_ts_clustering/models/temp_label')
    elif semi_sup == 1:
    ...
    for i in range(0,5):
        if semi_sup ==1:
            ...
            #model.load_state_dict(torch.load('/home/nahad/pycharm_mac/semi_with_cp/semi_ts_clustering/models/mackay_glass/semi_sup/20_labels_pairs'))
            model_label.load_state_dict(torch.load('/home/nahad/pycharm_mac/semi_with_cp/semi_ts_clustering/models/temp_label'))

            #model.load_state_dict(torch.load('/home/nahad/pycharm_mac/semi_with_cp/semi_ts_clustering/models/HCI/on_segmented_labels/semi_sup/free_hand_pair_cp_labels_1'))
            'pairswise cp rep that is good'
            #model.load_state_dict(torch.load('/home/nahad/pycharm_mac/semi_with_cp/semi_ts_clustering/models/wsdm/semi_sup/50_pairs_cp'))
        elif ae == 1:
            model_label.load_state_dict( torch.load('/home/nahad/pycharm_mac/semi_with_cp/semi_ts_clustering/models/temp_label'))
            modelAE.load_state_dict(torch.load('/home/nahad/pycharm_mac/semi_with_cp/semi_ts_clustering/models/temp_model_AE'))
        #model.load_state_dict(torch.load('/home/nahad/pycharm_mac/semi_with_cp/semi_ts_clustering/models/mackay_glass/semi_sup/No_labels_5'))
        for ep in range(0, 600):
            ...
            if ae == 1:
                ...
                #loss = train_ladder(ul_data_batch, balanced_train_btch,lam_ul,lam_l,ep)
                #acc = eval_ladder(label_val_btch,0)

                #writer.add_scalar('Loss training Ladder', loss, ep)
                #writer.add_scalar('Loss validation Ladder', acc, ep)
            elif semi_sup == 1:
                'Lambda for pairs from labels'
                lam_label = 1
                #lam_u = 0.001
                lam_u = 0
                ...
                if vis == 1:
                    acc = evaluate_classification(label_data_test,0,1)
            elif ladder_var == 1:
                lam_ul = 0.1
                lam_l = 0.1
                train_semi_sup_ladder(ul_data_batch, balanced_train_btch ,lam_ul,lam_l,ep)
                loss = eval_ladder(label_val_btch, lam_ul)
                writer.add_scalar('Validation loss : Ladder', loss, ep)

        #torch.save(model.state_dict(), '/home/nahad/pycharm_mac/semi_with_cp/semi_ts_clustering/models/wsdm/semi_sup/50_pairs_cp')
        #sys.exit()
        save_iters = 0
        if save_iters == 1:
            case_switch  ={1 : 'mean_var' , 2: 'ar', 4:'mackay_glass', 5:'wsdm', 6:'HCI'}

            case_string = case_switch[case]
            ...
            if semi_sup == 1:
                ...
                sub_fold = 'ladder'
            save_str = 'No_labels_{0}'.format(train_samples)
            if case == 6:
                save_string = os.path.join(model_save_base_path,case_string,'on_segmented_labels', sub_fold,save_str)
            else:
                save_string = os.path.join(model_save_base_path, case_string ,sub_fold, save_str)
            torch.load_state_dict(torch.load('/home/nahad/pycharm_mac/semi_with_cp/semi_ts_clustering/models/mackay_glass/semi_sup/20_labels_pairs'))


        #model.load_state_dict(torch.load('/home/nahad/pycharm_mac/semi_with_cp/semi_ts_clustering/models/HCI_pairs_freehand'))
        if semi_sup == 1:
            #train_feedforward_semisup(pair_train_btch, balanced_train_btch,lam_cp, lam_label)
            for ep in range(0,1000):
                ...
                #train_semi_entrop_reg_feedforward(balanced_train_btch, pair_train_btch, lam_l, lam_u)
                acc = evaluate_classification(label_val_btch, 0, 0)
                writer.add_scalar('Accuracy validation reg5', acc, ep)
                if case == 4:
                    acc = evaluate_classification(label_val_btch, 0, 0)
                    writer.add_scalar('Accuracy validation', acc, ep)
                #acc = evaluate_classification_uni_batch(label_data_test, 1, 1)


            writer.add_scalar('Accuracy validation', acc, ep)
            acc = evaluate_classification(label_data_test, 1,1)


            #acc_temp = evaluate_classification(label_data_test,1,1)
        elif ae == 1:
            acc_temp = evaluate_semisup_autoenc(label_data_test, 1, 0)
        elif ladder_var == 1:
            acc_temp = eval_semisup_ladder(label_data_test,1,1)
        val_list.append(acc_temp)
        #writer.add_scalar('Accuracy validation', acc, ep)
        print('accuracy: {0}'.format(acc))
        list_results.append(acc_temp)
